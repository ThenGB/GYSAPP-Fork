            ...
            if (hookEnabled("post:edit:console-warn", ["standard", "strict"]) &&
                ...
                if (content) {
                    ...
                    if (matches && matches.length > 0) {
                        log("warn", `[ECC] console.log found in ${filePath} (${matches.length} occurrence${matches.length > 1 ? "s" : ""})`);
                    }
                }
            }
        ...
        event: async ({ event }) => {
            switch (event.type) {
                ...
                case "file.watcher.updated": {
                    const file = event.properties.file;
                    const changeEvent = event.properties.event;
                    if (file) {
                        let changeType = "modified";
                        if (changeEvent === "add")
                            changeType = "added";
                        else if (changeEvent === "unlink")
                            changeType = "deleted";
