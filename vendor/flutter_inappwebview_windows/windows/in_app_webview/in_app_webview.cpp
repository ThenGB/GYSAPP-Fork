...
namespace flutter_inappwebview_plugin
{
    ...
    wil::com_ptr<ICoreWebView2CompositionController> webViewCompositionController)> completionHandler)
  {
    auto callback = [parentWindow, willBeSurface, completionHandler, initialSettings](HRESULT result, wil::com_ptr<ICoreWebView2Environment> env) -> HRESULT
      {
        // The environment callback can run on a WebView2 worker thread that
        // has no COM apartment initialized, which makes QueryInterface /
        // CreateCoreWebView2ControllerOptions fail with CO_E_NOTINITIALIZED
        // (error -2147221008). Initialize COM for the duration of the
        // callback so those calls succeed.
        ...
        // Only uninitialize when this callback initialized the apartment
        // (S_OK); S_FALSE means the thread already had an apartment that
  ...
  void InAppWebView::addWebNotificationController(const std::string& id_, std::shared_ptr<WebNotificationController> controller)
  {
    if (id_.empty() || !controller) return;
    webNotificationControllers_[id_] = std::move(controller);
  }

  WebNotificationController* InAppWebView::getWebNotificationController(const std::string& id_) const
  {
    auto it = webNotificationControllers_.find(id_);
    return it != webNotificationControllers_.end() ? it->second.get() : nullptr;
  }

  void InAppWebView::eraseWebNotificationController(const std::string& id_)
  {
    webNotificationControllers_.erase(id_);
  }
  ...
  void InAppWebView::addPrintJobController(const std::string& id_, std::shared_ptr<PrintJobController> controller)
  {
    if (id_.empty() || !controller) return;
    printJobControllers_[id_] = std::move(controller);
  }

  PrintJobController* InAppWebView::getPrintJobController(const std::string& id_) const
  {
    auto it = printJobControllers_.find(id_);
    return it != printJobControllers_.end() ? it->second.get() : nullptr;
  }

  void InAppWebView::erasePrintJobController(const std::string& id_)
  {
    printJobControllers_.erase(id_);
  }
  ...
  void InAppWebView::printCurrentPage(std::shared_ptr<PrintJobSettings> settings_,
    const std::function<void(const std::optional<std::string>&)> completionHandler)
  {
    ...
    bool showUI = settings_ && settings_->showUI.value_or(true);
    ...
    if (showUI) {
      ...
      if (settings_ && settings_->printDialogKind.has_value()) {
        dialogKind = static_cast<COREWEBVIEW2_PRINT_DIALOG_KIND>(settings_->printDialogKind.value());
      }
    ...
    if (settings_ && settings_->handledByClient.value_or(false)) {
      printJobId = get_uuid();
    }

    // Create print settings_ using PrintJobSettings helper method
    ...
    if (settings_) {
      printSettings = settings_->createPrintSettings(environment6.get());
    }
    else {
      // Create default print settings_ if no settings_ provided
    ...
    if (printJobId.has_value() && printJobManager) {
      printJobController = printJobManager->createPrintJobController(printJobId.value(), settings_);
  ...
  void InAppWebView::createPdf(std::shared_ptr<PrintJobSettings> settings_,
    const std::function<void(const std::optional<std::vector<uint8_t>>&)> completionHandler)
  {
    ...
    // Create print settings_ using PrintJobSettings helper method or default
    wil::com_ptr<ICoreWebView2Environment6> environment6;
    if (failedAndLog(webViewEnv->QueryInterface(IID_PPV_ARGS(&environment6))) || !environment6) {
      if (completionHandler) {
        completionHandler(std::nullopt);
  ...
  InAppWebView::~InAppWebView()
  {
    ...
    for (auto& [id_, channel] : webMessageChannels_) {
      if (channel) {
        channel->dispose();
      }
    }
